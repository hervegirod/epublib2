...
public class ContentPane extends JPanel implements NavigationEventListener,
   ...
   private static boolean matchesAny(String searchString, String... possibleValues) {
      for (int i = 0; i < possibleValues.length; i++) {
         ...
         if (!StringUtil.isEmpty(attributeValue) && (attributeValue.equals(searchString))) {
            return true;
         }
      }
      return false;
...
//					|| (keyEvent.getKeyCode() == KeyEvent.VK_DOWN)) {
   ...
   private String calculateTargetHref(URL clickUrl) {
      ...
      if (currentResource != null && !StringUtil.isEmpty(currentResource.getHref())) {
   ...
   public void navigationPerformed(NavigationEvent navigationEvent) {
      if (navigationEvent.isBookChanged()) {
         ...
         if (!StringUtil.isEmpty(navigationEvent.getCurrentFragmentId())) {
